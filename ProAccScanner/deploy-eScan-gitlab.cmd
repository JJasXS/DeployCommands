...
title ProAcc eScan Deploy (GitLab)

REM =============================================================================
REM ProAcc eScan (ProAccScanner) -- full Windows deploy from GitLab
...
set "APP_NAME=ProAccScanner"
set "EXE_NAME=FirebirdWeb.exe"
REM SSH form (often blocked): git@gitlabsvr.oneclickclouds.com:softwaredevelopment/ProAccScanner.git
set "REPO_URL=https://gitlabsvr.oneclickclouds.com/softwaredevelopment/ProAccScanner.git"
set "GIT_BRANCH=main"
set "CSPROJ=FirebirdWeb.csproj"

set "WORK_DIR=C:\Source\ProAccScanner"
set "APP_DIR=C:\Apps\ProAccScanner"
set "HTTP_PORT=8443"
set "ENV_PREP=%TEMP%\ProAccScanner.env.prepared"
set "ENV_BACKUP=%TEMP%\ProAccScanner.env.backup"
set "FW_RULE_NAME=eScan %HTTP_PORT%"
REM ----------------------------------------------
...
if errorlevel 1 (
  ...
  ) > "%TEMP%\elevate_escan.vbs"
  wscript //nologo "%TEMP%\elevate_escan.vbs"
  if errorlevel 1 (
    echo.
...
cd /d "%~dp0"
echo.
echo [%DATE% %TIME%] eScan deploy GitLab (Administrator)
echo Script: %~f0
echo Folder: %~dp0
echo.
...
) else if exist "C:\Temp\ProAccScanner.env.backup" (
  copy /Y "C:\Temp\ProAccScanner.env.backup" "%ENV_PREP%" >nul
...
copy /Y "%ENV_PREP%" "C:\Temp\ProAccScanner.env.backup" >nul
...
echo [3/10] Wipe old source / app folders...
...
sc description "%APP_NAME%" "ProAcc eScan / ProAccScanner (ASP.NET Core self-contained)."
