...
title SqlAccountSync Deploy (GitLab)

REM =============================================================================
REM SqlAccountSync (multiDBSync) -- full Windows deploy from GitLab
...
REM
REM ASPNETCORE_URLS must stay in .env so the Windows service listens on 5030
REM (launchSettings.json is not used by sc create).
REM
REM Keep validate-env.ps1 in the SAME folder as this .cmd
REM =============================================================================

REM ------------ EDIT FOR EACH CLIENT ------------
set "APP_NAME=SqlAccountSync"
set "EXE_NAME=SqlAccountSync.exe"
REM SSH form (often blocked on port 22): git@gitlabsvr.oneclickclouds.com:softwaredevelopment/multiDBSync.git
set "REPO_URL=https://gitlabsvr.oneclickclouds.com/softwaredevelopment/multiDBSync.git"
set "GIT_BRANCH=master"
set "CSPROJ=src\SqlAccountSync\SqlAccountSync.csproj"

set "WORK_DIR=C:\Source\SqlAccountSync"
set "APP_DIR=C:\Apps\SqlAccountSync\publish"
set "HTTP_PORT=5030"
set "ENV_PREP=%TEMP%\SqlAccountSync.env.prepared"
set "ENV_BACKUP=%TEMP%\SqlAccountSync.env.backup"
set "FW_RULE_NAME=SqlAccountSync %HTTP_PORT%"
REM ----------------------------------------------
...
if errorlevel 1 (
  ...
  ) > "%TEMP%\elevate_sqlaccountsync.vbs"
  wscript //nologo "%TEMP%\elevate_sqlaccountsync.vbs"
  if errorlevel 1 (
    echo.
...
cd /d "%~dp0"
echo.
echo [%DATE% %TIME%] SqlAccountSync deploy GitLab (Administrator)
echo Script: %~f0
echo Folder: %~dp0
echo.
...
echo.
echo Keep ASPNETCORE_URLS=http://0.0.0.0:%HTTP_PORT% so the service binds correctly.
...
) else if exist "C:\Temp\SqlAccountSync.env.backup" (
  copy /Y "C:\Temp\SqlAccountSync.env.backup" "%ENV_PREP%" >nul
  echo Prefill: copied from C:\Temp\SqlAccountSync.env.backup
) else (
  (
    echo # SqlAccountSync / multiDBSync - tenant API URL no longer defaults in appsettings.json
    echo TENANT_CODE=
    ...
    echo ASPNETCORE_URLS=http://0.0.0.0:5030
    echo.
    echo # Optional central Firebird control DB ^(app starts without it; /health still works^)
    echo # CentralDb__Path=C:/SQLData/SqlAccountSync/SYNC.FDB
    echo # CentralDb__PasswordSecretRef=env:CENTRAL_FB_MASTERKEY
    echo # CENTRAL_FB_MASTERKEY=masterkey
  ) > "%ENV_PREP%"
...
if not exist "%ENV_VALIDATOR%" set "ENV_VALIDATOR=C:\Temp\validate-SqlAccountSync-env.ps1"
...
if errorlevel 1 (
  echo.
  echo .env is incomplete. Fix the required values and run this script again.
  goto :fail
)

copy /Y "%ENV_PREP%" "%ENV_BACKUP%" >nul
copy /Y "%ENV_PREP%" "C:\Temp\SqlAccountSync.env.backup" >nul
...
REM Prefer real SDK exe (avoids a repo-local dotnet.cmd swallowing control).
...
if not exist "C:\Apps\SqlAccountSync" mkdir "C:\Apps\SqlAccountSync"
...
pushd "%WORK_DIR%"
"%DOTNET_EXE%" clean "%CSPROJ%" -c Release >nul
if exist "src\SqlAccountSync\bin" rmdir /s /q "src\SqlAccountSync\bin"
if exist "src\SqlAccountSync\obj" rmdir /s /q "src\SqlAccountSync\obj"
...
"%DOTNET_EXE%" publish "%CSPROJ%" -c Release -r win-x64 --self-contained true -o "%APP_DIR%" /p:RazorCompileOnBuild=true
if errorlevel 1 (
  popd
...
sc description "%APP_NAME%" "SQL Account Sync Background Service"
