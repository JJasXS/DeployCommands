...
title ProAcc eQuotation Deploy (GitLab)

REM =============================================================================
REM ProAcc eQuotation -- full Windows deploy from GitLab
...
REM SSH form (often blocked on port 22): git@gitlabsvr.oneclickclouds.com:softwaredevelopment/eQuotation.git
set "REPO_URL=https://gitlabsvr.oneclickclouds.com/softwaredevelopment/eQuotation.git"
set "GIT_BRANCH=main"
...
if errorlevel 1 (
  ...
  wscript //nologo "%TEMP%\elevate_equotation.vbs"
  if errorlevel 1 (
    echo.
...
cd /d "%~dp0"
echo.
echo [%DATE% %TIME%] eQuotation deploy GitLab (Administrator)
echo Script: %~f0
echo Folder: %~dp0
echo.
...
if exist "%APP_DIR%\.env" (
  ...
  (
    echo # ProAcc eQuotation - tenant API URL no longer defaults in appsettings.json
    echo TENANT_CODE=
